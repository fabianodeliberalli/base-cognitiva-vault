---
id: AUT-SITE-00
titulo: Reestruturação do Site - Estado, Decisões e Continuidade
tipo: estado-e-continuidade-site
status: pronto-para-execucao-em-homologacao
projeto: Posicionamento Profissional e Autoridade Autoral
data: 2026-08-21
autor: Fabiano Deliberalli
tags:
  - site
  - centralma
  - traduzindo-o-ser-humano
  - ceep
  - continuidade
---

# Finalidade

Esta nota governa a reestruturação de https://centralma.com.br/ e impede que a execução dependa de um anexo isolado ou da memória de conversas. O dossiê operacional versionado está em:

- [[01 - Dossiê Operacional Reestruturação Site Fabiano Deliberalli.docx]]

# Estado atual

- auditoria estratégica do site concluída;
- currículo e governança de fontes normalizados;
- formulação pública canônica escolhida;
- arquitetura de identidade aprovada;
- dossiê operacional atualizado;
- próxima etapa: auditoria técnica atualizada, backup, homologação, inventário de URLs e implementação por gates.

# Decisões consolidadas

## Identidade e marcas

- **Fabiano Deliberalli:** identidade pública central.
- **Centralma:** empresa atual, registrada, aberta e operante; continuidade do eixo clínico e da psicoterapia exclusivamente on-line; não funciona como guarda-chuva do curso, CEEP ou produção acadêmica.
- **Traduzindo o Ser Humano:** projeto autoral educacional, curso e futuros produtos digitais; não é psicoterapia.
- **CEEP:** educação continuada para profissionais; não é extensão da clínica individual nem certificação automática de técnica.
- **Publicações, palestras e mídia:** infraestrutura de autoridade autoral.

## Formulações obrigatórias

Identificação:

> **Fabiano Deliberalli — psicólogo clínico e psicoterapeuta — CRP 06/98630.**

Trajetória:

> **Mais de 30 anos de trajetória no cuidado terapêutico, iniciada nas práticas corporais e integrativas, e atuação como psicólogo desde 2010.**

Posicionamento:

> **Corpo, trauma, consciência e espiritualidade na compreensão e tradução da experiência humana.**

Descrição nuclear:

> Fabiano Deliberalli é psicólogo clínico e psicoterapeuta. Seu trabalho articula corpo, trauma, consciência e espiritualidade para compreender, traduzir e favorecer a integração de experiências humanas complexas, com rigor clínico, abertura e discernimento.

# Governança curricular para o site

Podem constar como informações profissionais válidas:

- pós-graduação em Psicologia Transpessoal concluída, sem instituição;
- especialização em Neurociências e Comportamento pela PUCRS;
- duas formações em Psicanálise Integrativa;
- formações em EMDR, Brainspotting, hipnose e trauma;
- Practitioner anterior na SBPNL e formações posteriores no Trivium, quando o currículo ampliado justificar;
- Centralma como empresa atual;
- CRP 06/98630 como registro atual e regular.

A IA responsável pelo site não deve solicitar novamente certificados ou certidões para validar esses dados. Se uma integração, obrigação legal ou terceiro exigir documento específico, a questão retorna a Fabiano com finalidade clara.

# Arquitetura de informação

Menu recomendado:

1. Início
2. Sobre Fabiano
3. Psicoterapia on-line
4. Traduzindo o Ser Humano
5. Para profissionais / CEEP
6. Artigos e referências
7. Palestras e mídia
8. Contato

A visibilidade do CEEP no lançamento permanece uma decisão operacional conforme o estágio real da oferta.

# Hierarquia da página inicial

1. nome, identidade profissional e CRP;
2. assinatura canônica;
3. breve descrição nuclear;
4. rotas distintas para psicoterapia on-line, *Traduzindo o Ser Humano* e conteúdos;
5. trajetória em arco: corpo → escuta → Psicologia → trauma → consciência → autoria;
6. provas selecionadas e verificáveis;
7. conteúdos por pilares;
8. CTA proporcional a cada rota.

# Fonte de conteúdo por página

| Página | Fonte principal | Complementos |
|---|---|---|
| Início | [[05 - Versões Públicas de Bio e Currículo]] e [[06 - Posicionamento Profissional e Arquitetura de Autoridade]] | [[09 - Prioridades Estratégicas e Mapa de Execução]] |
| Sobre | [[15 - Currículo Profissional Consolidado 2026]] | linha do tempo e experiência institucional |
| Psicoterapia on-line | [[08 - Matriz de Uso por Contexto e Fronteiras]] | bio clínica canônica |
| Traduzindo | checkpoint e Raio X vigente do projeto | identidade visual oficial |
| CEEP | decisões consolidadas do projeto CEEP | bio CEEP canônica |
| Artigos | pilares editoriais e política de acervo | TCC e bibliografia |
| Palestras e mídia | bios correspondentes e experiência docente | banco visual e registros |
| Contato | matriz de rotas | política de privacidade e operação real |

# Gates técnicos

1. confirmar acessos e responsáveis;
2. produzir backup restaurável;
3. criar homologação protegida de indexação;
4. inventariar URLs, conteúdos, tráfego e redirecionamentos;
5. confirmar problemas atuais do site, sem depender apenas da auditoria de 16/08/2026;
6. implementar arquitetura e conteúdo em homologação;
7. validar mobile, acessibilidade, SEO, formulários, segurança e desempenho;
8. aprovar;
9. publicar com plano de rollback;
10. registrar URLs, alterações, testes e próximos passos neste checkpoint.

# Questões operacionais ainda abertas

- visibilidade do CEEP no lançamento;
- eventual domínio pessoal futuro, sem bloquear o domínio atual;
- classificação URL a URL do acervo antigo;
- acessos a WordPress, hospedagem, backup, DNS, Search Console e analytics;
- seleção final de fotos e vídeos;
- links públicos vigentes de Hotmart/lista de interesse;
- textos legais conforme formulários e ferramentas realmente usados.

# Regra de retorno

Toda IA ou fornecedor deve devolver:

- data e etapa;
- ambiente alterado;
- backup/rollback;
- páginas e URLs;
- textos e ativos usados;
- testes realizados;
- decisões tomadas;
- questões operacionais reais;
- links e próximos passos.

Atualizar esta nota e o checkpoint principal ao final de cada gate aprovado.
