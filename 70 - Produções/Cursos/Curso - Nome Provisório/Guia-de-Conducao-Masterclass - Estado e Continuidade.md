---
tipo: estado-e-continuidade-masterclass
projeto: Traduzindo o Ser Humano
status: vigente
versao: "1.0"
criado_em: 2026-08-23
atualizado_em: 2026-08-23
autor: Fabiano Deliberalli
---

# Guia de Condução da Masterclass — Estado e Continuidade

## Documento vigente

- [[Guia-de-Conducao-Masterclass-Traduzindo-o-Ser-Humano-v2.6]]
- **Estatuto:** canônico para condução;\n- **SHA-256:** `065fd70c28e67ed6e8fe67aea5bdf3aac177001b59b24fad93353374b3690d8c`;
- **Condição de uso ao vivo:** correção visual do slide 16 e ensaio operacional integral;
- **Estrutura pública apresentada:** 9 módulos e 54 aulas;
- **Rastreabilidade do conteúdo:** arquitetura 7/14, Nota-Ponte e Matriz Mestre.

## Decisões protegidas

- título: “Por que você entende seus padrões, mas continua repetindo?”;
- big idea: “Saber não é ainda ter acesso”;
- história composta de demora de resposta e medo de rejeição como narrativa pedagógica principal;
- demonstração ao vivo como experiência educativa, não prova;
- episódio autobiográfico do Sorriso Interior como segmento breve, singular e ilustrativo;
- práticas corporais e energéticas, Psicologia, clínica, ciência, filosofia e espiritualidade integradas sem neutralização;
- trajetória no cuidado terapêutico com mais de 30 anos e atuação como psicólogo clínico desde 2010;
- EIXO como função pedagógica interna;
- oferta de 9 módulos, 54 aulas, acesso de um ano, condições de preço sujeitas ao checkout e garantia de sete dias.

## Pendências reais antes da transmissão

1. alterar no deck o rótulo do slide 16 para “O EIXO como síntese do percurso” ou equivalente;
2. realizar ensaio integral com cronômetro;
3. verificar checkout, link, recursos e condições da oferta na data da transmissão.

A versão Markdown v1.1 permanece histórica e não governa a condução atual.

## Relações

- [[07 - Deliberação Transversal - Posicionamento, Linguagem e Arquitetura Dupla - 2026-08-23]]
- [[05 - Nota-Ponte - Desenvolvimento do Conteúdo 7-14 e Entrega 9-54]]
- [[06 - Matriz Mestre de Incorporação do Conteúdo à Estrutura 9-54]]
