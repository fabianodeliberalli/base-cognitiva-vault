---
tipo: protocolo-de-continuidade-integral
projeto: Traduzindo o Ser Humano
curso: nome-provisorio
status: vigente
versao: "1.4"
autor: Fabiano Deliberalli
criado_em: 2026-07-21
atualizado_em: 2026-07-21
fonte_persistente: fabianodeliberalli/base-cognitiva-vault
branch: main
tags:
  - continuidade
  - curso
  - governanca
  - handoff
  - arquitetura-pedagogica
---

# Protocolo de Continuidade Integral e Abertura de Novos Chats

> **Entrada obrigatória:** iniciar por [[LEIA PRIMEIRO - Estado Atual e Continuidade do Curso]]. Esta nota declara o estado vigente, a hierarquia documental e o ponto exato de retomada.

## 1. Finalidade e garantia operacional

Este documento é a ponte oficial entre chats para a construção do curso pertencente ao projeto Traduzindo o Ser Humano.

Sua finalidade é permitir que um novo chat retome o trabalho sem reconstruir decisões, sem reabrir deliberações já concluídas e sem depender da memória de uma conversa anterior.

Continuidade integral não significa transportar literalmente cada frase de todos os chats. Significa preservar, com rastreabilidade:

- a identidade do projeto e do curso;
- as decisões aprovadas;
- o estatuto de cada formulação;
- a arquitetura do Método EIXO;
- a arquitetura curricular da primeira oferta;
- a pedagogia e os limites éticos;
- os ativos produzidos;
- as fontes e auditorias;
- a reserva curricular disponível e seus gatilhos de retomada;
- o Inventário Mestre de Conteúdos Disponíveis;
- as questões realmente abertas;
- o estado operacional mais recente;
- a próxima ação autorizada.

A fonte persistente é o repositório GitHub sincronizado com o Obsidian:

- repositório: fabianodeliberalli/base-cognitiva-vault;
- branch: main;
- pasta do curso: 70 - Produções/Cursos/Curso - Nome Provisório.

Toda consulta às notas do Obsidian deve ser feita por esse repositório.

## 2. Regra de autoridade e prevalência documental

A continuidade deve obedecer a duas hierarquias simultâneas.

### 2.1. Autoridade institucional e conceitual

1. Constituição Filosófico-Pedagógica do Projeto Traduzindo o Ser Humano;
2. Manual Institucional do Sistema Operacional;
3. Protocolo Operacional para Construção da Arquitetura de Cursos v1.0;
4. Dossiê de Contexto e Arquitetura Mestre do curso;
5. Guia Mestre de Incorporação e Rastreabilidade Curricular;
6. Registro Mestre de Formulações Estruturantes;
7. documentos metodológicos consolidados do Método EIXO;
8. arquitetura curricular e índices-mestres;
9. kits das aulas;
10. fontes, auditorias e patrimônio;
11. propostas, hipóteses e protótipos.

### 2.2. Atualidade operacional

Para saber em que ponto o trabalho realmente está, prevalece o documento mais específico e mais recente dentro de seu escopo:

0. [[LEIA PRIMEIRO - Estado Atual e Continuidade do Curso]], para o estado transversal mais recente;
1. Registro de Produção Visual e QA ou registro pós-turma da aula;
2. Índice do Kit da aula;
3. Índice Mestre dos Kits;
4. Protocolo de Continuidade Integral;
5. Dossiê de Contexto e Arquitetura Mestre.

Consequência: o Dossiê orienta a arquitetura geral, os princípios e as decisões do curso. O Índice do Kit e o Registro de QA da Aula 1.1 prevalecem quanto ao estado atual da produção dessa aula.

Uma nota inferior não pode alterar uma decisão institucional superior. Uma nota superior mais antiga também não pode ser usada para negar a existência de um artefato ou estágio operacional posteriormente documentado em seu registro específico.

Em caso de conflito:

- preservar a decisão superior;
- adotar o estado operacional mais recente;
- registrar a divergência;
- não apagar silenciosamente o histórico;
- solicitar deliberação de Fabiano somente se houver contradição material não resolvível pela regra acima.

## 2.3. Autoridade, disponibilidade, função e momento

Aprovação e disponibilidade não são sinônimos. Todo conteúdo pertinente permanece no Acervo Disponível para Construção, salvo exclusão autoral explícita ou restrição ética, jurídica, de confidencialidade ou de direitos.

Cada ativo deve ser lido em quatro eixos independentes:

1. **autoridade:** decisão aprovada, formulação consolidada, em revisão, proposta, hipótese, fonte, protótipo, versão superada ou excluído por decisão;
2. **disponibilidade:** ativo, sob demanda, reserva longitudinal, em curadoria, restrito ou excluído;
3. **função pedagógica:** transversal, participante, educador, aprofundamento, prática, mapa, metáfora, caso, bibliografia, segurança ou outra oferta;
4. **momento:** contrato, transversal, aula definida, retomada progressiva, educador, piloto, pós-piloto ou oferta futura.

O estado padrão de disponibilidade é **sob demanda**, nunca indisponível. O campo `validado_por_fabiano: false` indica decisão pendente, não proibição de análise. “Reservado” restringe o momento da entrega, não a consideração arquitetônica.

Ver [[01 - Reconciliação da Governança do Conhecimento e Disponibilidade do Acervo]] e [[02 - Inventário Mestre de Conteúdos Disponíveis]].

## 3. Identidade do projeto

O projeto Traduzindo o Ser Humano tem como núcleo a Tradução da Experiência Humana.

Seu valor singular não é apenas integrar disciplinas, mas oferecer linguagem, diferenciação e arquitetura para experiências humanas que frequentemente permanecem sem nome adequado, fragmentadas entre corpo, afeto, narrativa, relação, história, valores, espiritualidade e diferentes sistemas explicativos.

Campos como psicologia, neurociência, fenomenologia, tradições contemplativas, Medicina Chinesa e espiritualidade são tratados como mapas de um território humano comum. Não são verdades concorrentes nem equivalentes automáticos.

A ciência participa como fundamento transversal, linguagem de convergência, descrição e correlação proporcional à evidência. Não ocupa o lugar de tribunal único ou principal da experiência humana. Clínica, fenomenologia, filosofia, símbolo, tradição, espiritualidade, experiência e produção autoral permanecem legítimos segundo o tipo de afirmação e o modo de uso. O rigor recai sobre o enunciado empregado, não sobre a classificação total de um autor.

A pedagogia do projeto parte do vivido, torna a experiência legível, favorece orientação e retorna à vida concreta. Seu horizonte é autonomia, discernimento, integração progressiva e expressão coerente, sem impor metafísica única.

## 4. Identidade atual do curso

O curso permanece com nome provisório.

Natureza aprovada:

- estrutura educacional complementar;
- primeira expressão pedagógica e comercial derivada da arquitetura integral do Método EIXO;
- curso-piloto formativo integral;
- percurso pelas seis fases em profundidade fundamental;
- campo de validação pedagógica, ética e experiencial, não de validação clínica ou científica.

Tese central:

> Compreender uma experiência não garante capacidade de sustentar e integrar aquilo que ela continua mobilizando.

Problema humano central aprovado:

> Este curso existe porque pessoas que já percorreram trajetórias de autoconhecimento, psicoterapia, contemplação ou espiritualidade continuam vivendo, no presente, os efeitos de experiências passadas em diferentes dimensões de sua experiência — corporal, afetiva, relacional, narrativa, existencial e, quando pertinente, espiritual. Frequentemente interpretam que precisam compreender ainda mais, encontrar outra técnica ou alcançar uma explicação definitiva, quando necessitam de uma arquitetura educacional complementar que transforme o repertório já acumulado em capacidade de reconhecer, sustentar, discernir e começar a integrar aquilo que essas experiências continuam mobilizando.

Transformação nuclear:

- passar de repertório predominantemente compreendido ou narrado para capacidade inicial incorporada;
- reconhecer o que acontece;
- sustentar ou recuperar presença possível;
- diferenciar e traduzir;
- discernir;
- orientar respostas;
- reparar quando necessário;
- reorganizar-se e retornar ao percurso.

O curso não promete cura, integração completa, estabilidade permanente, resolução de trauma, realização espiritual ou confirmação metafísica.

## 5. Público

Público principal:

- adultos com trajetória prévia em autoconhecimento, psicoterapia, práticas contemplativas, espiritualidade ou religião;
- pessoas que já possuem repertório, mapas, técnicas ou explicações;
- pessoas que percebem distância entre o que sabem e o que conseguem sustentar quando a experiência se torna mobilizadora;
- participantes dispostos a exercícios leves, graduados, facultativos e não expositivos.

Formulação estratégica do público:

> Pessoas com trajetória prévia que continuam vivendo efeitos multidimensionais de experiências passadas e precisam de uma arquitetura para transformar o que já compreenderam em capacidade de reconhecer, sustentar e integrar no presente.

Persona candidata derivada da mentoria com Camila e da leitura de Joe Dispenza:

> A buscadora experiente cartograficamente fragmentada.

Ela possui vários mapas, interesse por ciência, energia, espiritualidade e prática, mas carece de arquitetura para distinguir, relacionar e incorporar o que sabe. Camila confirma fortemente o perfil cultural e pedagógico, mas não deve ser tratada como persona exclusiva nem como prova isolada do problema humano central.

Para quem o curso não é:

- quem busca diagnóstico ou tratamento;
- quem busca resolução imediata;
- quem quer confirmação de uma cosmologia;
- quem espera intervenção clínica individual;
- quem não pode ou não deseja participar de práticas leves mesmo com alternativas.

Hipóteses comerciais ainda em revisão:

- entrega ao vivo pelo Zoom e posterior hospedagem na Hotmart;
- curso principal antes de um workshop de menor valor;
- faixa de valor aproximada de R$ 997;
- curso como acelerador no eixo Canal → Curso → Comunidade.

Essas hipóteses não devem determinar artificialmente profundidade, promessa ou duração.

## 6. Método EIXO

Definição nuclear aprovada:

> O Método EIXO é a expressão metodológica do projeto Traduzindo o Ser Humano destinada a desenvolver a capacidade incorporada de sustentar presença diante da própria experiência, organizá-la com maior discernimento e ampliar progressivamente o espaço entre impulso e resposta, tendo autonomia como horizonte formativo.

Competência nuclear ampliada:

> Capacidade progressiva de reconhecer a própria experiência, sustentar ou recuperar presença diante dela, organizá-la com discernimento e ampliar a liberdade de orientar respostas mais coerentes com o contexto, os valores e aquilo que a pessoa reconhece como essencial em si.

EIXO não é estado permanente. É função dinâmica de orientação, retorno e reorganização.

Critérios de aprendizagem:

- reconhecer afastamento;
- pausar;
- recuperar presença possível;
- reencontrar orientação;
- escolher resposta mais coerente;
- reparar consequências quando necessário;
- reorganizar-se progressivamente.

Princípio de Recursividade EIXO–Tradução:

> Algum EIXO favorece a Tradução da experiência; ao torná-la mais legível, a Tradução pode favorecer reorganização, e essa reorganização pode fortalecer o EIXO. A relação é circular, dinâmica e progressiva, não linear nem automática.

Quatro movimentos funcionais:

1. Tornar-se presente — reconhecer estado, pausar, encontrar apoio e recuperar contato possível;
2. Tornar a experiência legível — perceber, diferenciar, traduzir e revisar interpretações;
3. Habitar a experiência — sustentar contato sem fusão total, fuga ou imposição prematura de sentido;
4. Orientar a resposta — consultar contexto e valores, escolher, responder, reparar e retornar.

Seis fases aprovadas:

1. Traduzir a Experiência;
2. Regular e Recuperar Presença;
3. Observar e Diferenciar;
4. Fortalecer o EIXO;
5. Integrar Progressivamente;
6. Expressar e Contribuir.

Jornada Fenomenológica:

> Traduzir → Aprender a Ler → Habitar → Estabilizar → Encarnar.

Perceber é condição transversal de abertura e recorrência. Presença é capacidade variável, não requisito de perfeição. Reparação e aprendizagem formam movimento de retorno, com maior explicitação na Fase 6.

As seis fases são recursivas. Retornar não é regredir.

## 7. Governança lexical e espiritualidade

Distinções vigentes:

- self psicológico: organização dinâmica da subjetividade;
- Self junguiano: totalidade psíquica e individuação no referencial junguiano;
- Essência: horizonte experiencial autoral de autenticidade, potência, valores e direção;
- Alma: linguagem espiritual, filosófica, tradicional ou poética contextualizada;
- Eu Superior: função tradutiva do Mapa dos Três Eus;
- EIXO: função operacional aprovada de presença, discernimento, orientação e retorno;
- Fonte: linguagem experiencial, tradicional, filosófica ou hipótese cosmológica conforme o contexto.

Nenhum termo é sinônimo automático dos demais.

O Mapa dos Três Eus é um mapa funcional transversal do projeto, não requisito estrutural do Método:

- Eu Básico: sustentação corporificada e acesso ao implícito;
- Eu Médio: mediação reflexiva, simbolização, Tradução e escolha;
- Eu Superior: sentido, valores, direção e contribuição;
- EIXO: coordenação, orientação e retorno.

Teste de removibilidade: o Método precisa permanecer compreensível, praticável e avaliável sem a nomenclatura dos Três Eus.

Linguagens energéticas podem entrar como experiência, símbolo, tradição ou hipótese explicitamente qualificada. Não são apresentadas como descrição científica objetiva.

## 8. Pedagogia obrigatória

O curso deve:

- começar pela experiência, antes da teoria;
- oferecer contato manejável com o vivido;
- diferenciar manifestação, interpretação, hipótese, tradição, símbolo e correlação científica;
- preservar autoria e direito de não adotar uma linguagem;
- usar exposição breve, demonstração, prática dosada, reflexão e aplicação;
- distinguir conhecimento declarativo, competência incorporada e sabedoria prática;
- manter ciência como tradução proporcional à evidência;
- evitar falsas equivalências;
- favorecer autonomia, discernimento e transferência para a vida;
- avaliar capacidade de retorno, não ausência de desorganização;
- declarar o estatuto de mapas, metáforas, protótipos e fontes.

Fluxo editorial de uma aula:

1. experiência humana reconhecível;
2. pergunta ou tensão;
3. mudança de paradigma;
4. mapa ou fundamento apenas quando necessário;
5. tradução do vivido;
6. prática facultativa e dosada;
7. integração;
8. retorno ao presente;
9. ponte para a próxima aula.

Contrato formativo:

- autonomia;
- dosagem;
- possibilidade de pausar;
- alternativas à prática;
- direito de não compartilhar;
- não interpretação entre participantes;
- não exposição obrigatória;
- proteção de dados;
- não diagnóstico;
- não substituição de cuidados profissionais.

## 9. Arquitetura curricular da primeira oferta

Estado: proposta de prototipagem fundada em decisões estruturais aprovadas.

Formato candidato:

- 7 módulos, contando o Módulo 0;
- 14 aulas nucleares;
- primeira turma ao vivo;
- gravação e eventual hospedagem;
- caderno de práticas, fichas e mapas;
- validação progressiva após piloto.

Estrutura:

| Módulo | Função | Aulas candidatas | Evidência principal |
|---|---|---|---|
| 0 | Orientação e contrato | 0.1 Tradução da Experiência Humana; 0.2 EIXO, recursividade e segurança | reconhece limites e forma de participação |
| 1 | Traduzir a Experiência | 1.1 A experiência em sua multidimensionalidade; 1.2 Do vivido à Tradução | distingue manifestação, interpretação e hipótese |
| 2 | Regular e Recuperar Presença | 2.1 Corpo, ritmos e afastamento; 2.2 Recursos de retorno e dosagem | reconhece sinais e recupera presença possível |
| 3 | Observar e Diferenciar | 3.1 Perceber, presença e observação; 3.2 Padrões, partes e contextos | diferencia componentes do vivido |
| 4 | Fortalecer o EIXO | 4.1 Valores, contexto e essencial; 4.2 Espaço entre impulso e resposta | formula orientação e amplia escolha |
| 5 | Integrar Progressivamente | 5.1 Recursividade Tradução–EIXO; 5.2 Conectar partes sem apagar diferenças | relaciona dimensões e sustenta tensões |
| 6 | Expressar e Contribuir | 6.1 Expressão, limites e contribuição; 6.2 Reparação, aprendizagem e continuidade | transfere, repara e retorna |

Os títulos e a distribuição das aulas 2.1 a 6.2 ainda são protótipos curriculares, não texto congelado.

## 10. Organização obrigatória dos kits

Cada aula deve possuir, conforme pertinência:

1. ficha pedagógica;
2. roteiro do educador;
3. contrato e cuidados;
4. material do participante;
5. exemplos e casos didáticos;
6. mapas e imagens;
7. storyboard e apresentação;
8. critérios de pilotagem e revisão;
9. registro pós-turma.

Decisões aprovadas permanecem na ficha pedagógica. Texto de fala pertence ao roteiro. Material do participante permanece separado. Mapas declaram estatuto. Alterações pós-turma passam primeiro pelo registro de pilotagem.

## 11. Estado dos kits

### Aula 0.1

Conteúdo anterior existe, mas ainda precisa ser organizado no padrão oficial do kit. Não reconstruir do zero sem auditar o material existente.

### Aula 0.2 — Percurso do EIXO

Kit documental organizado e apresentação visual produzida.

Núcleos:

- EIXO como capacidade de reconhecer e retornar;
- seis fases como organização do ensino;
- quatro movimentos transversais;
- sentir–perceber como abertura;
- presença não binária;
- retornar não é regredir;
- espiral e pêndulo como aprendizagem com oscilação;
- efeitos, responsabilidade, reparação e aprendizagem;
- contrato de autonomia, dosagem e pluralidade;
- linha de base não diagnóstica.

### Aula 1.1 — A experiência em sua multidimensionalidade

Estado operacional vigente em 21 de julho de 2026:

- protótipo visual concluído;
- 22 slides renderizados e inspecionados;
- ficha do participante com 4 páginas;
- roteiro do educador v1;
- deliberações consolidadas;
- banco de exemplos;
- folha dos estatutos das linguagens;
- storyboard;
- plano visual;
- critérios de pilotagem;
- mapa Semente–Casca–Campo;
- registro de produção visual e QA.

Estado exato:

> Protótipo historicamente aprovado para pilotagem controlada em 21 de julho de 2026, mas reaberto por deliberação posterior para revisão e reorganização curricular antes da pilotagem; não aprovado para gravação ou primeira oferta.

Decisões pedagógicas da Aula 1.1:

- versão integral ao vivo proposta em 100 minutos;
- futura versão gravada em blocos menores;
- exemplo principal: mensagem breve e ambígua de alguém importante;
- experiência pessoal apenas leve ou moderada;
- expressão simbólica opcional;
- partilha em duplas, voluntária, sem interpretação;
- perguntas fenomenológicas;
- retorno corporal e registro de recurso ao final.

Estrutura visual:

1. abertura e pergunta nuclear;
2. contrato de segurança e escolha;
3. horizonte integral;
4. mapa das dimensões;
5. exemplo da mensagem ambígua;
6. manifestações corporais, afetivas, narrativas e relacionais;
7. Sentir–Perceber–Observar–Traduzir;
8. Semente–Casca–Campo;
9. vários mapas para o mesmo território;
10. estatutos das linguagens;
11. Cartografia das Cinco Janelas;
12. prática individual;
13. expressão simbólica opcional;
14. partilha sem interpretação;
15. integração e retorno;
16. relação circular Tradução–EIXO.

Próxima ação autorizada:

> Realizar a revisão e reorganização curricular da Aula 1.1 sob o Guia Mestre, P2 e o núcleo mínimo de P6. Somente depois submeter a nova versão à deliberação autoral para pilotagem.

## 12. Mapas e patrimônio visual

Mapas estruturantes e ativos:

- Mapa das Dimensões da Experiência Humana;
- Modelo dos Sete Níveis;
- Cartografia das Cinco Janelas;
- Abertura Multidimensional Responsável;
- Sentir–Perceber–Observar–Traduzir;
- vários mapas para um território humano;
- EIXO dinâmico;
- Centro, Campo, Borda e Sombra;
- Mandala, Labirinto e EIXO;
- Arquitetura Plural de Validação;
- Semente, Casca e Campo;
- árvore espiral como metáfora candidata;
- mapas do lote de 11 imagens, classificados e ainda não automaticamente estruturantes.

Regra: mapa estruturante aprovado, síntese autoral, correlação hipotética, metáfora e protótipo devem permanecer diferenciados.

Conteúdos visuais a produzir apenas no momento curricular pertinente:

- sombra, persona, complexos e projeção — Fase 3;
- mandala longitudinal — Fase 4;
- labirinto, direção e valores — Fase 4 ou 5;
- borda, expressão e contribuição — Fase 6;
- comparação longitudinal de mandalas ou cartografias — encerramento.

## 13. Patrimônio ABBSP

A apresentação ABBSP março de 2026 possui:

- 459 slides;
- 85 recursos visuais;
- 41 slides vazios;
- repetições e formulações experimentais.

Estatuto:

> Patrimônio autoral bruto de alta relevância, não apresentação final nem currículo automático. Permanece disponível por inteiro e por extrações, conforme função e momento.

Extrações organizadas:

1. Diferenciação de Níveis sem Colapso;
2. Semente, Casca, Campo e Árvore;
3. Sentir, Interocepção e Coração;
4. Fronteira, Ego Funcional e Permeabilidade;
5. Ressonância Estrutural e Campo Relacional;
6. Três Regulações, Qi Gong e EIXO;
7. Unidade Diferenciada e Espiritualidade Integrada;
8. Diferenciação Clínica de Experiências Espirituais.

Incorporação na Aula 1.1:

- diferenciação de níveis;
- semente–casca–campo;
- sentir e interocepção;
- pluralidade de estatutos.

Reserva para fases posteriores ou formação de educadores:

- Princípio da Energia Livre;
- Markov blanket;
- Rede Tripartite;
- Tan Tien;
- Jung aprofundado;
- alquimia taoista;
- diferenciação clínica;
- emergência espiritual;
- fronteiras e ego funcional;
- ressonância estrutural;
- práticas tradicionais progressivas.

Correções epistemológicas permanentes:

- correlação não é equivalência;
- imagem cerebral não é prova;
- tradição não é ciência estabelecida;
- experiência não comprova causa;
- hipótese cosmológica não é fundamento científico;
- testemunho não prova causalidade;
- diagnóstico diferencial não pertence ao curso geral.

## 14. Fonte Joe Dispenza e Camila

A obra Como se tornar sobrenatural integra o acervo como fonte bibliográfica em diálogo. Seu uso é definido pela contribuição concreta de cada conceito, prática ou formulação, não por um rótulo global atribuído ao autor.

Contribuições possíveis:

- momento presente;
- interrupção de automatismos;
- condicionamento corporal e emocional;
- respiração;
- atenção corporal;
- ensaio mental;
- prática regular;
- abertura à transformação;
- perfil cultural do público.

Estatuto das utilizações possíveis:

- centros energéticos: linguagem tradicional e experiencial;
- circulação de energia: experiência, tradição ou hipótese;
- manifestação quântica: não usar como ciência estabelecida;
- pineal e DMT: não usar como mecanismo comprovado;
- testemunhos de cura: não usar como evidência causal;
- campos globais: não usar como comprovação;
- práticas meditativas: graduadas, optativas e com alternativas.

Função estratégica da obra:

- evidencia demanda do público;
- mostra mapas reconhecíveis;
- revela a necessidade de Tradução e critérios;
- não governa a arquitetura do Método.

Pendência documentada: cópia manual do PDF para a pasta de anexos bibliográficos, caso ainda não tenha sido concluída.

## 15. Reserva curricular disponível e fila de retomada

Os conteúdos abaixo permanecem disponíveis desde já para consulta, correlação e coerência longitudinal. Sua entrega deve ocorrer no momento pedagógico pertinente:

- sombra, persona, complexos, projeção e Jung aprofundado;
- Self junguiano e individuação;
- mandala longitudinal;
- Três Eus como mapa auxiliar;
- Tan Tien, Três Regulações e Qi Gong;
- Medicina Chinesa e alquimia taoista;
- Rede Tripartite, sincronia neural interpessoal e ressonância relacional;
- Princípio da Energia Livre, inferência ativa e Markov blankets;
- interocepção, respostas evocadas pelo coração e corpo preditivo;
- limites, ego funcional, borda e permeabilidade;
- experiências liminares e abertura ontológica responsável;
- dignidade epistêmica;
- transformação versus integração;
- disclosure e choque ontológico;
- unidade diferenciada;
- espiritualidade integrada;
- reparação, contribuição e incorporação;
- diferenciação clínica de experiências espirituais para formação profissional;
- livro Arquitetura Invisível;
- livro do Sistema Límbico-Telepático;
- TCC de Sincronia Neural Interpessoal;
- monografia de Psicologia Transpessoal;
- materiais de Medicina Chinesa;
- casos clínicos anonimizados;
- metáforas, práticas e mapas candidatos.

Todo conteúdo deve passar pelos testes de necessidade, função, estatuto da afirmação, segurança e momento curricular. O teste de removibilidade verifica se um elemento é estruturalmente indispensável; não decide se ele pode ser consultado ou utilizado. Não antecipar a entrega não significa impedir consideração arquitetônica.

## 16. Questões realmente abertas

Não tratar como resolvido:

- nome e subtítulo do curso;
- formato definitivo;
- carga horária total;
- duração final de cada encontro;
- modalidade final e regime de gravação;
- valor e estratégia comercial;
- condições finas de entrada e exclusão;
- instrumentos definitivos de acompanhamento;
- validação do público por evidência além de Camila;
- títulos finais e distribuição das aulas 2.1 a 6.2;
- estatuto final da Matriz Integrada v0.1;
- lugar final dos Três Eus na primeira oferta;
- seleção das práticas tradicionais;
- versão final do léxico pedagógico reduzido;
- execução, registro e deliberação pós-piloto da Aula 1.1;
- organização completa da Aula 0.1;
- inserção manual do PDF de Dispenza, se pendente.

Não reabrir sem contradição real:

- problema humano central;
- natureza educacional complementar;
- competência nuclear do EIXO;
- EIXO como capacidade de retorno;
- quatro movimentos;
- seis fases;
- recursividade;
- Perceber e presença como transversalidades;
- reparação e aprendizagem como retorno;
- primeira oferta integral de prototipagem;
- limites éticos e epistemológicos;
- estado de QA da Aula 1.1.

## 17. Manifesto mínimo e busca temática ampliada

O manifesto abaixo é o núcleo mínimo para orientação rápida, não a totalidade do patrimônio. Toda tarefa temática deve complementar a leitura por meio do [[02 - Inventário Mestre de Conteúdos Disponíveis]] e de busca ampliada no repositório.

Leitura obrigatória inicial:

1. [[00 - Dossiê de Contexto e Arquitetura Mestre]];
2. [[01 - Protocolo de Continuidade Integral e Abertura de Novos Chats]];
3. [[02 - Matriz Integrada - Quatro Movimentos, Seis Fases e Jornada Fenomenológica]];
4. [[09 - Esqueleto Estrutural da Primeira Oferta Integral para Mentoria]];
5. [[00 - Índice Mestre dos Kits de Aula]];
6. [[Aula 0.2 - Índice do Kit]];
7. [[Aula 1.1 - Índice do Kit]];
8. [[02 - Roteiro do Educador v1]];
9. [[08 - Critérios de Pilotagem v1]];
10. [[09 - Registro de Produção Visual e QA v1]].

Leitura contextual antes de alterar a Aula 1.1:

- [[01 - Deliberações de Produção]];
- [[03 - Ficha Cartografia Multidimensional Inicial v1]];
- [[04 - Banco de Exemplos v1]];
- [[05 - Folha Estatutos das Linguagens v1]];
- [[06 - Storyboard dos Slides v1]];
- [[07 - Plano Visual e Mapas v1]];
- auditoria ABBSP;
- Matriz de Extração ABBSP;
- auditoria bibliográfica de Joe Dispenza;
- Perfil de Público — Camila e o Leitor de Dispenza;
- Registro Visual — Jung, Símbolos e Validação Plural.

Consultar conforme necessidade, não como leitura obrigatória integral para toda tarefa:

- Léxico Mestre;
- Mapa Funcional Transversal dos Três Eus;
- auditorias de mapas;
- fundamentos científicos;
- patrimônio autoral bruto;
- fontes bibliográficas;
- reserva curricular disponível e seus gatilhos de retomada.

Antes de declarar ausência de conteúdo ou produzir do zero, consultar o Inventário Mestre, trabalhos autorais, ABBSP, núcleo NHI, fontes bibliográficas, mapas, versões históricas e Dívidas de Organização relacionadas.

## 18. Protocolo de trabalho do novo chat

Antes de produzir:

1. acessar o repositório e ler primeiro [[LEIA PRIMEIRO - Estado Atual e Continuidade do Curso]], seguida do manifesto mínimo;
2. emitir Relatório de Retomada;
3. distinguir aprovado, consolidado, candidato, hipótese, protótipo, pendência e arquivo histórico;
4. confirmar o ponto operacional atual;
5. não reabrir decisões aprovadas sem contradição documentada;
6. não produzir material de outra aula antes de verificar o kit correspondente.

Durante a produção:

- trabalhar em uma tarefa delimitada;
- preservar o Dossiê como orientação geral;
- preservar o índice e o QA da aula como estado operacional;
- registrar fontes e estatutos;
- separar material do educador e do participante;
- registrar conteúdos descobertos fora de foco como Dívidas de Organização;
- não interromper o fluxo da aula para desenvolver patrimônio de fases futuras.

Ao encerrar:

1. atualizar o Índice do Kit;
2. atualizar o registro de QA, pilotagem ou pós-turma;
3. atualizar o Índice Mestre se o estado mudou;
4. registrar decisões, pendências e próxima ação;
5. registrar novas Dívidas de Organização com os sete campos oficiais;
6. sincronizar tudo no GitHub;
7. informar o ponto exato de retomada.

## 19. Arquitetura recomendada de chats

Recomendação: abrir o novo chat dentro do mesmo projeto do ChatGPT, porque isso preserva instruções e contexto geral disponíveis ao projeto. Ainda assim, o GitHub/Obsidian permanece a fonte oficial.

Não usar um único chat infinito para todo o curso.

Adotar estrutura híbrida:

### Chat mestre

Nome sugerido:

> Curso — Orquestração Mestre 02

Função:

- decisões transversais;
- arquitetura global;
- público, promessa e formato;
- governança;
- integração entre módulos;
- atualização do Dossiê;
- aprovação de mudanças que afetam mais de um módulo;
- passagem de estado entre chats.

### Chats de produção por módulo

Abrir somente quando o módulo entrar em produção real:

- Curso — Módulo 0 — Orientação;
- Curso — Módulo 1 — Traduzir a Experiência;
- Curso — Módulo 2 — Regular e Recuperar Presença;
- Curso — Módulo 3 — Observar e Diferenciar;
- Curso — Módulo 4 — Fortalecer o EIXO;
- Curso — Módulo 5 — Integrar Progressivamente;
- Curso — Módulo 6 — Expressar e Contribuir.

Cada chat de módulo:

- lê este Protocolo;
- lê o Dossiê;
- lê a Matriz Integrada;
- lê o esqueleto curricular;
- lê o índice do próprio módulo e os kits de suas aulas;
- não altera decisões transversais sozinho;
- registra qualquer necessidade transversal para o Chat Mestre;
- encerra cada sessão atualizando o repositório.

Não abrir todos os chats de uma vez. Criá-los conforme o módulo entra em produção reduz dispersão.

A Aula 1.1 está no Módulo 1 e possui um protótipo completo produzido em etapa anterior. P2 foi aprovado integralmente e o núcleo mínimo de P6 foi aprovado. O Chat Mestre deve agora conduzir a revisão e reorganização curricular da Aula 1.1, sem tratá-la como canônica ou protegida. A versão resultante deverá voltar à deliberação autoral antes da pilotagem.

## 20. Texto de abertura para copiar e colar no novo chat

Este chat dará continuidade integral à construção do curso pertencente ao projeto Traduzindo o Ser Humano.

A fonte oficial de continuidade é o repositório GitHub sincronizado com meu Obsidian:

- fabianodeliberalli/base-cognitiva-vault
- branch main
- pasta 70 - Produções/Cursos/Curso - Nome Provisório

Toda consulta às notas do Obsidian deverá ser feita por esse repositório.

Antes de produzir, modificar ou deliberar:

1. Leia integralmente:
- LEIA PRIMEIRO - Estado Atual e Continuidade do Curso.md;
- 00 - Dossiê de Contexto e Arquitetura Mestre.md;
- Guia Mestre de Incorporação e Rastreabilidade Curricular.md;
- Registro Mestre de Formulações Estruturantes.md;
- Registro Consolidado da Sessão - Governança e Arquitetura Curricular - 2026-07-21.md;
- 01 - Protocolo de Continuidade Integral e Abertura de Novos Chats.md;
- 02 - Matriz Integrada - Quatro Movimentos, Seis Fases e Jornada Fenomenológica.md;
- 09 - Esqueleto Estrutural da Primeira Oferta Integral para Mentoria.md;
- 60 - Arquitetura Pedagógica/00 - Índice Mestre dos Kits de Aula.md;
- 60 - Arquitetura Pedagógica/Aula 1.1 - A experiência em sua multidimensionalidade/00 - Índice do Kit.md;
- na mesma pasta da Aula 1.1: 02 - Roteiro do Educador v1.md, 08 - Critérios de Pilotagem v1.md e 09 - Registro de Produção Visual e QA v1.md.

2. Examine todos os arquivos vinculados no Índice da Aula 1.1, incluindo PowerPoint, ficha, storyboard, plano visual, mapas, fontes incorporadas e deliberações.

3. Aplique esta regra de autoridade:
- o Dossiê orienta a arquitetura geral;
- o Protocolo de Continuidade resolve a passagem entre chats;
- o Índice Mestre organiza os kits;
- o Índice e o Registro de QA da Aula 1.1 prevalecem quanto ao estado operacional atual;
- documentos institucionais superiores continuam prevalecendo quanto a princípios e limites.

4. Diferencie obrigatoriamente:
- aprovado;
- consolidado;
- candidato;
- hipótese;
- protótipo;
- pendência;
- patrimônio histórico;
- fonte bibliográfica;
- reserva curricular disponível.

5. Não reabra decisões consolidadas sem demonstrar contradição real e solicitar minha deliberação.

6. Apresente primeiro um Relatório de Retomada com:
- identidade e estado do curso;
- arquitetura vigente;
- público e transformação;
- estrutura curricular;
- estado de cada kit existente;
- estado exato da Aula 1.1;
- materiais disponíveis;
- decisões aprovadas;
- hipóteses e questões abertas;
- divergências documentais encontradas;
- próxima ação recomendada.

7. Não altere documentos antes de eu aprovar o Relatório de Retomada.

Ponto atual confirmado em 21 de julho de 2026:

A Aula 1.1 — A experiência em sua multidimensionalidade possui protótipo visual concluído, PowerPoint de 22 slides, ficha do participante de 4 páginas, mapas, roteiro, storyboard, critérios de pilotagem e Registro de QA. A aprovação de 21 de julho para pilotagem permanece como registro histórico; o estado vigente reabre a aula para revisão e reorganização curricular antes do piloto. Ela não está aprovada para gravação ou primeira oferta.

P2 está aprovado integralmente e o núcleo mínimo de P6 está aprovado. A frente ativa é revisar a Aula 1.1 como aula normal da arquitetura vigente, utilizando seu acervo como matéria-prima e não como referência canônica. Depois da reorganização, apresentar a nova versão para deliberação autoral antes da pilotagem.

Ao final de cada etapa, atualize no GitHub/Obsidian o estado, as decisões, as pendências, as Dívidas de Organização e o próximo ponto exato de retomada.

## 21. Critério de continuidade plena

A continuidade é considerada plena quando o novo chat consegue responder, com fonte documental:

- por que o curso existe;
- para quem existe;
- qual transformação busca;
- como se relaciona ao Método EIXO;
- quais decisões não podem ser reabertas;
- quais conteúdos entram agora;
- quais conteúdos compõem a reserva curricular disponível e seus gatilhos;
- quais arquivos já existem;
- qual artefato governa cada decisão;
- em que estado está a Aula 1.1;
- qual é a próxima ação;
- onde registrar tudo ao terminar.

Se uma dessas respostas depender apenas da memória do chat, há uma deficiência de continuidade e ela deve ser registrada e corrigida no repositório.


## 22. Controle de versão 1.4

A versão 1.4 registra a aprovação integral de P2, a aprovação do núcleo mínimo de P6 e a reclassificação vigente da Aula 1.1 como protótipo revisável, reaberto para escrutínio e reorganização antes da pilotagem.

A versão 1.3 registrou a disponibilidade das propostas v0.1 de P2 e P6, deslocando o ponto de retomada de formulação para deliberação autoral e preservando os portões como abertos até decisão expressa.

A versão 1.2 cria vínculo obrigatório com [[LEIA PRIMEIRO - Estado Atual e Continuidade do Curso]], integra o [[Registro Consolidado da Sessão - Governança e Arquitetura Curricular - 2026-07-21]] e reconcilia as duas frentes atuais: P2/P6 como trabalho curricular ativo e pilotagem da Aula 1.1 como trabalho operacional pendente. A Aula 1.2 permanece condicionada aos dois portões e à deliberação pós-piloto.

A versão 1.1 incorporou a Reconciliação da Governança do Conhecimento e Disponibilidade do Acervo, substitui o uso geral de canônico/não canônico, transforma o conteúdo futuro em reserva curricular disponível, torna obrigatória a busca temática ampliada, integra o Inventário Mestre e atualiza a Aula 1.1 para seu estado vigente de aprovação para pilotagem controlada.
