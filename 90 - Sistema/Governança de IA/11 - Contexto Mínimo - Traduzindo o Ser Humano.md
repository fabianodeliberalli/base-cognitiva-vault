---
tipo: ficha-de-contexto-minimo
projeto: Traduzindo o Ser Humano
status: referencia-atual-revisavel
versao: "0.6"
atualizado_em: 2026-09-07
---

# Contexto mínimo — Traduzindo o Ser Humano

Use esta ficha para começar. Expanda a leitura somente conforme a tarefa. Todas as formulações abaixo são referências atuais de trabalho e podem ser revistas por Fabiano.

## Referências atuais de trabalho

- **Traduzindo o Ser Humano** é a marca organizadora e o nome atual da oferta.
- **EIXO** é função pedagógica interna de presença, discernimento, orientação e retorno; não é automaticamente produto, mecanismo comercial ou promessa.
- **9 módulos / 54 aulas** são a arquitetura usada atualmente. Esse formato ainda será estruturado para posterior gravação e pode evoluir por decisão de Fabiano.
- A organização inicial de **7 módulos / 14 aulas** não é mais estrutura. Seus títulos e conteúdos permanecem como acervo histórico de consulta.
- Durante a estruturação das 54 aulas para posterior gravação, o acervo inicial pode ser consultado como fonte qualificada para **ampliações, correlações e fundamentações**. Cada contribuição será escolhida por necessidade e pertinência ao conteúdo da aula, sem correspondência obrigatória, sem transportar a numeração antiga e sem restaurar a estrutura inicial como arquitetura.
- Quando **2.1** ou **2.2** identificarem antigas aulas ou unidades, são apenas códigos históricos. Todas as variantes de títulos e conteúdos devem ser preservadas; nenhuma é aula vigente. Elas podem ser consultadas individualmente conforme pertinência, sem determinar numeração, posição ou ordem de gravação nas 54 aulas.
- O subtítulo usado atualmente é **Da compreensão acumulada à presença que sustenta**; ele permanece revisável por Fabiano.
- Espiritualidade é dimensão explícita, afirmativa, plural e não dogmática. A identidade integrada de Fabiano não deve ser neutralizada por heurísticas genéricas.
- “Competência incorporada” é conceito de aprofundamento; não deve virar headline fria por padrão.
- O curso não promete cura, integração completa, estabilidade permanente, resolução de trauma, realização espiritual ou confirmação metafísica.
- Fabiano pode aprovar, rejeitar, combinar ou revisar qualquer escolha autoral a qualquer momento. Nenhuma inteligência artificial pode declarar uma proposta própria como final, canônica, congelada ou imutável.

## Duração ainda em construção

- **Doze semanas** é somente uma estimativa provisória recuperada de versões anteriores.
- A duração atual não está definida de forma definitiva.
- Ela dependerá da construção posterior das 54 aulas, de seu volume, ritmo, práticas, materiais, forma de liberação e outros fatores ainda em curso.
- Nenhum assistente deve transformar a estimativa em promessa, regra de oferta ou cronograma sem nova decisão de Fabiano.

## Estado operacional resumido

- Memória persistente: este repositório GitHub sincronizado com o Obsidian.
- No momento, a produção curricular concentra-se na frente de trabalho M1.1–M1.7, que permanece revisável conforme a construção das aulas.
- UC 0.1 possui uma versão atual de referência; audiovisual e gravação permanecem etapas distintas, e Fabiano pode reabrir o conteúdo quando isso contribuir para o curso.
- UC 0.2/M1.7 é uma fonte atual para derivação audiovisual; seu conteúdo continua disponível para revisão, ampliação e novas relações.
- Aula 1.1 permanece protótipo de trabalho totalmente revisável.
- Quando houver divergência material, considerar a demanda atual e a escolha mais recente de Fabiano; consultar apenas se a diferença realmente mudar o resultado.

## Roteamento por tarefa

| Demanda | Consultar primeiro |
|---|---|
| Estado e próxima ação | `LEIA PRIMEIRO - Estado Atual e Continuidade do Curso.md` e estado da frente ativa |
| Identidade, limites e promessa | Dossiê; diretriz de espiritualidade e não neutralização |
| Aproveitamento do acervo inicial de 7 módulos e 14 aulas | Documentos históricos, Nota-Ponte e Matriz Mestre de Incorporação, apenas como fontes de conteúdo |
| Produção de M1 | Estado da Produção M1 e artefatos exatos da aula solicitada |
| Formulações atuais e histórico de desenvolvimento | Registro Mestre de Formulações Estruturantes |
| Fonte ou conteúdo possivelmente existente | Inventário Mestre e busca temática no acervo |
| Histórico de uma decisão | protocolo, deliberação ou registro de sessão correspondente |
| Atrito repetido ou melhoria do fluxo | `12 - Aprendizado Operacional e Melhoria Contínua.md`, somente sob demanda |

## Regras de carga

Não ler automaticamente todos os documentos da tabela. Escolher apenas a linha pertinente. Documentos extensos, transcrições, montagens e fontes são recuperados sob demanda.

Se a tarefa não exigir mudança autoral, não transformar a resposta em nova deliberação.
